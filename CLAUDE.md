...
4. **อ่านข้อมูลด้วย Server Components, เขียนด้วย Server Actions** — บันทึกผลเทสผ่าน Server Action; ผูกกับ `auth.uid()` **เสมอ** — ตอนไม่ล็อกอินใช้ `signInAnonymously()` (ได้ uid ชั่วคราว), ตอนล็อกอิน upgrade เป็นบัญชีถาวรด้วย `linkIdentity()`/`updateUser()` → **ผลเทสเดิมตามไปเอง (uid ไม่เปลี่ยน)** (ดู 7.2)
...
| `riasec_track_map` | `holland_code`, `track_id`, `weight`, `reason` | map → สาย |
| `riasec_career_map` | `holland_code`, `career_id`, `weight`, `reason` | map → อาชีพ |
| `riasec_major_map` | `holland_code`, `major_id`, `weight`, `reason` | map → สาขา |

> ตาราง `*_map` เก็บ **`reason`** ไว้อธิบายว่า "ทำไม Holland code นี้ถึงเหมาะกับสิ่งนี้" — เนื้อหานี้คือหัวใจของฟีเจอร์ ต้องมีเสมอ
...
- **Auth = optional login ผ่าน Anonymous Sign-in**:
  - ตอนเริ่มทำเทส (ไม่ล็อกอิน) เรียก `signInAnonymously()` → ได้ `auth.uid()` จริง (RLS ทำงานได้); ล็อกอินภายหลังใช้ `linkIdentity()` → uid เดิมคงอยู่ ผลเทสตามไปเอง
...
4. **Test UI** — flow ทำแบบทดสอบ (mobile-first, progress, ทีละคำถาม); เรียก **Anonymous Sign-in** ตอนเริ่มทำเทส
